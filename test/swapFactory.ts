...
describe("Swap Factory", function () {
  ...
  it("Should create new swap contract", async function () {
    ...
    const tx = await SwapFactoryOwner.createSwapContract(startDate, endDate, 2, 10, 10000, 100, 1, false, 1000, {startTime: 7,unlockInterval: 30, percentageToMint: 10});
  ...
  it("Should fail if createSwapContract is called with invalid owner", async function () {
    ...
    await expect(
        swapFactory.createSwapContract(startDate, endDate, 2, 10, 10000, 100, 1, false, 1000, {startTime: 7,unlockInterval: 30, percentageToMint: 10})
      ).to.be.rejectedWith(
